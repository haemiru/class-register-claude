...
-- 1) classregi_registrations 컬럼 추가
alter table classregi_registrations add column if not exists email text;
alter table classregi_registrations add column if not exists form_data jsonb not null default '{}'::jsonb;
